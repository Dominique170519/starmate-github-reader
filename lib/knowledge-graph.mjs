...
export function buildKnowledgeGraph(packages = []) {
  ...
  const changes = packages.flatMap((learningPackage) => {
    const history = learningPackage.changeHistory?.length
      ? learningPackage.changeHistory
      : learningPackage.changeSummary ? [learningPackage.changeSummary] : [];
    return history.map((change) => ({
      packageId: learningPackage.id,
      fullName: learningPackage.fullName,
      sourceSha: change.sourceSha || learningPackage.sourceSha,
      sourceUpdatedAt: change.sourceUpdatedAt || learningPackage.sourceUpdatedAt,
      ...change,
